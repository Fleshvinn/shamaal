...
Function stuff()
{
	...
	print "<div align=center><a href=admin.php?load=$load&action=$action&search_place=$search_place&search_typ=$search_typ&search_specif=$search_specif&search_name=$search_name&search_level=$search_level&page=$page&action=add class=menu>Добавить</a></div>";
	$sear = "";
	if ($search_place > -1)
	...
	for ($i=0;$i<$count;$i=$i+10)
	{
		...
		if ($i == $page)
			...
			$p .= "|<a href=admin.php?load=$load&action=$action&search_place=$search_place&search_typ=$search_typ&search_specif=$search_specif&search_name=$search_name&search_level=$search_level&page=$i class=menu>$i-$e</a>| ";
	}
...
<tr><td>Вампиризм</td><td><input type=text name=drain_attack value='$drain_attack' size=3 maxlength=3></td></tr>
			...
			<tr><td>Одет</td><td>$on".($active == 1 ? " [<a href=\"admin.php?load=$load&id=$id&do=unwearitem&name=$pl_name&action=$action&sid=$sid&show_obj=1\">Снять</a>]" : "")."</td></tr>
...
Function player()
{
	...
	if (isset($name))
	{
		...
		$SQL="select id,chp,cmana,name,pic,sex,race,str,dex,intt,wis,con,h_up,exp,level,gold,bank_gold,s_up,city,city_rank,city_pay,city_text,online,ban,ban_for,admin_text,reg_ip,ip,admin,reg_date,city,clan,pack,city_rank,clan_rank, ban_chat, mail , skill_down, up_login, room, resp_room from sw_users where up_name='$name'";
		$row_num=SQL_query_num($SQL);
		while ($row_num){
			$pl_id = $row_num[0];
			$pl_chp = $row_num[1];
...
Function news()
{
	...
	while ($row_num){
		...
		$com_text = "$com_text<table width=450 align=center bgcolor=92A7AB>";
			...
			$com_text = "$com_text<tr>";
				$com_text = "$com_text<td colspan=2 align=right>";
					$com_text = "$com_text <input type=\"checkbox\" name=\"post_is_important\" value=\"1\"$post_important_flag> Важная | <a href=admin.php?load=1&action=del&id=$post_id>Удалить</a>&nbsp;<input type=submit value=Сохранить class=but>";
				$com_text = "$com_text</td>";
			$com_text = "$com_text</tr>";
		$com_text = "$com_text</table></form><br>";
		if ($action == "gen")
