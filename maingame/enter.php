...
if ((isset($ebar)) && ($ebar > ''))
	...
	{	
		...
		$ebar = str_replace('"',"`",$ebar);
		$ebar = str_replace('shamaal.ru', 'swlands.ru', $ebar);
		$ebar = str_replace('шамаал.ру', 'swlands.ru', $ebar);
	...
	//						print "$SQL";
		...
		if (strtoupper($comm) == "/TELEPORT")
		{
			...
			if ($param[0] > 0)
			{
				...
				if ($result)
					mysql_free_result($result);
				//print "$n_admin";
				if (($n_admin == 1) || ($n_admin == 4) || ($n_admin == 2) || ($n_admin == 7))
 				{
		...
		if (strtoupper($comm) == "/TELEPORTTO")
		{
			...
			if ($param[0]  != "")
			{
				...
				//print "$n_admin";
				if (($n_admin == 1) || ($n_admin == 4) || ($n_admin == 7))
 				{
